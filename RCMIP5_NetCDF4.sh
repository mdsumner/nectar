...
Rscript -e 'x <- .libPaths();install.packages(c("abind", "plyr", "reshape2", "foreach", "digest", "ggplot2", "ncdf4", "ncdf", "doParallel", "fields", "testthat", "knitr"), lib = x[length(x)-1], repos = "http://cran.csiro.au")'

## 3) Install RCMIP5
Rscript -e 'x <- .libPaths();install.packages("RCMIP5", lib = x[length(x)-1], repos = "http://cran.csiro.au")'
